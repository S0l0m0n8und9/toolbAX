...
/// <summary>
/// Embedded-browser interactive sign-in for the Compare plugin. Mirrors the Operations plugin's
/// window: it drives the Data Integrator portal and watches its network traffic to capture, in one
/// flow, the delegated access/refresh token (from the Entra token-endpoint response) and the
/// regional gateway host (from the first <c>projectmanagementservice</c> request). The capture
/// logic lives in the shared <see cref="DualWriteSignInCapture"/> (FoToolbox.Core); this WPF shell
/// is per-plugin because the plugins load in isolated assembly contexts.
/// </summary>
