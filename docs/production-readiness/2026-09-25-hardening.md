...
| H08b | Use one shared request URL normalization policy across profile and connection paths. | In progress | Focused RED observed the raw `/data` duplication, a relative Catalog request for a bare host, non-canonical authority spelling, an unnormalized portal identifier, and bearer omission for padded profile URLs. Focused `CI=true` Release GREEN: Avalonia 50/50, Core 29/29, and origin guard/auth handler 17/17. Full `CI=true` Release validation passed with 0 warnings/errors: Avalonia 1,014/1,014 and Core 373/373. | Required before H01 resumes; not complete until its separate PR is merged and review evidence is recorded. |
