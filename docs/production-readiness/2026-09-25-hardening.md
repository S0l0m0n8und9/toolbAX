...
| H08b | Use one shared request URL normalization policy across profile and connection paths. | In progress | Focused RED observed the raw `/data` duplication, a relative Catalog request for a bare host, non-canonical authority spelling, an unnormalized portal identifier, and bearer omission for padded profile URLs. Greptile PR217 P1 found that the v2 key hid legacy `UserImport` Tables rows; its follow-up found that lowercased legacy keys cannot safely distinguish custom path casing. The focused recovery/cache suite is green at 37/37. Full `CI=true` Release revalidation passed with 0 warnings/errors: Avalonia 1,014/1,014 and Core 391/391; ignored logs/TRX are saved under `artifacts/`. | Auto-recovery is limited to deserializable `UserImport` Tables rows on an http/https authority root (including `/data` root aliases). A custom path, query, fragment, or userinfo throws a safe explicit-reimport exception carrying the original JSON but never writes a v2 row; legacy metadata/embedded tables remain untouched. Required before H01 resumes; not complete until its separate PR is merged and review evidence is recorded. |
