...
|---|---|---|---|---|
| H01 | Keep environment identity coherent when switching environments or editing profile URL/auth data; discard in-flight results and invalidate environment-scoped cached tools. | In progress | Source premises verified. Locked addendum: carry a deterministic versioned `EnvironmentIdentity` SHA-256 partition on `FoEnvironment.MetadataCachePartition`; partition every Catalog metadata/XML/memo/ETag/lock key while leaving Tables/imports/H08b migration and unpartitioned v2 callers unchanged; tag all Catalog HTTP requests and make `AuthenticatedHttpHandler` reject tagged pre-auth/post-auth context drift before dispatch; validate app-only F&O/Dataverse principals against the captured profile after lookup. Acceptance requires real `CatalogService`/temporary `CatalogStore` restart tests with same-ETag distinct documents, tagged-handler zero-dispatch gates, and an A-B-A principal-lookup test with no wrong broker invocation. Implementation remains paused for H08b merge. | Pending |
