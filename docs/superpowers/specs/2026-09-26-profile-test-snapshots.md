...
Parent full validation at unchanged source `f7ee086b3cbe55908e7110b1e0e55f812d061759` built the CI=true Release App solution with WebView2 enabled and zero warnings/errors, then passed 1,473/1,473 App tests with zero failures/skips. Core was unchanged by diff, so the prior 456/456 Core gate remains applicable. Evidence is `artifacts/h08a/pr229-review/parent-full-app-{build,test}.log` and `parent-full-app.trx`. No live services were used.

## PR #229 review correction 2: same-profile selection replacement

Greptile finding `4109500697` in thread `PRRT_kwDOQmkX-M6mMEoM` against exact head `19ba8539e511ae6bb5c7a90c21f37599d07d2daf` identified that Save replaces `Selected` with a new record for the same profile ID. The selection callback therefore cleared `LegacyDiStatus` even though the user had not changed profile. The callback now receives old/new records and clears the confirmation only when their IDs differ ordinally; draft loading, typed-secret clearing, gateway-result invalidation and presence notifications remain unchanged.

Attached rendering exposed an additional framework detail: replacing the selected record in `ObservableCollection` makes the Avalonia `ListBox` transiently publish a null selection before the updated same-ID record is assigned. Save therefore captures the same-ID owner's confirmation immediately before list replacement and restores it only when final `Selected` again owns that same ID. Genuine different-profile and null selections still clear the confirmation. Behavioral RED failed the headless and attached same-profile Save cases while 65 controls passed. GREEN passed 67/67, including different/null controls. The CI=true Release App solution then built with WebView2 enabled and zero warnings/errors; the prior 305-test Profiles/H07/H08/render/Shell baseline plus the new control passed 306/306 with zero failures/skips. Evidence is `artifacts/h08a/pr229-review/pr229-review2-behavioral-red-2.{log,trx}`, `pr229-review2-regression-green-2.{log,trx}`, `pr229-review2-app-build.log`, and `pr229-review2-focused-final.{log,trx}`. No live services or full-suite claim is included; parent review and the full App gate remain pending for this second correction.

Parent full validation at unchanged source `84f169f3ec95d6820df9ee20632c2f7c418e55f7` built the CI=true Release App solution with WebView2 enabled and zero warnings/errors, passed 1,474/1,474 App tests with zero failures/skips, and confirmed Core unchanged so the prior 456/456 Core gate remains applicable; evidence is `artifacts/h08a/pr229-review/parent-review2-full-app-{build,test}.log` and `parent-review2-full-app.trx`.
