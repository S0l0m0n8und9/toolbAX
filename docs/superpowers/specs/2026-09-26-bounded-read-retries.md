...
A separately scoped follow-up can evaluate bounded body staging or a streaming parse that preserves retry ownership, strict collection validation, final-response-only yield and cancellation truth. No peak-memory measurement or OOM reproduction exists in the current evidence. Ben accepted the present large-page memory limitation for PR234 and approved tracking that follow-up separately. D06 remains an open technical item; this disposition does not claim the memory limitation is fixed or authorize an implementation in the current retry PR.
