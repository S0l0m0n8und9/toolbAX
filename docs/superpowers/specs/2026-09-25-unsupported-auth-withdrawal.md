...
| A user enters a new secret before saving a legacy-to-ClientSecret draft. | The still-legacy service-principal blob is overwritten even if profile replacement is later declined, and the later accepted save deletes the new secret as incompatible. | Enable secret entry/store only when the saved profile is already ClientSecret and draft mode, effective client ID and tenant still match it. Commands repeat the guard before storage; fake and real SQLite/vault sequences cover decline, accepted replacement, subsequent storage and unrelated save. |
