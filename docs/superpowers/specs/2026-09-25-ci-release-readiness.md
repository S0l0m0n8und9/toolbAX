...
Implemented and locally validated after H15 integration; PR, hosted CI, review and merge remain pending. No signing, package/SDK
...
Evidence is retained under ignored `artifacts/h04/`: `smoke-integrated-green.trx`, `audit-shape-{red,green}.log`, `shipping-audit-final.json`, `actionlint-final.log`, `native-package-final.log`, and `package-final/smoke-dc4e6de8134447adbee1e545bfbe34fb/report.json`. This proves local startup/package readiness only. Hosted Windows WebView2 availability, Linux/Windows remote jobs and release publication are not claimed by local validation. Parent full and integration gates are recorded below. Distribution remains unsigned and requires an available WebView2 Runtime.

Parent independently passed both CI=true Release solution builds with zero warnings/errors at `1be3599ff6dfd6f820ab1869d0f37340bd34d4c3`, Windows App with EnableWebView2=true 1,214/1,214 and Core 432/432, zero failed/skipped. Evidence: `artifacts/h04/parent-{app,core}-{restore,build,test}.log` and `parent-{app,core}-full.trx`. H15 is now integrated from `af6e434a1010d0cbec6a2d9ae43ca0743094b41e`; the fresh native package smoke and parent final App gates passed at integration source `58b3d11764f35048f1028e8e5bfcccf235669f6b`. Hosted CI and publication remain unexecuted at this checkpoint.
At integrated source `58b3d11764f35048f1028e8e5bfcccf235669f6b`, parent CI=true Windows App restore/build passed with zero warnings/errors and all 1,214 tests passed, zero failed/skipped (`integrated-app-{restore,build,test}.log`, `integrated-app-full.trx`). Core code/tests are unchanged from the parent 432/432 gate at `1be3599`. The fresh extracted-package report confirms all three Release assembly identities match the integration SHA, real startup readiness and WebView2 availability. Report: `artifacts/h04/package-h15-integrated/smoke-dbeddd024f024a8cb176f82459b6f04c/report.json`; log: `native-package-h15-integrated.log`. Parent independently matched zip/checksum SHA256 `d701f0cc11cdca685bc4da1dd6c13868f8ed204fffeb7d61414844041fa2acf9`. PR/hosted CI/review/merge remain pending; no publication was executed.
## Release-label correction

The validated prerelease regex group now determines the release label, with the existing major-zero prerelease policy retained. A hyphen in build metadata alone does not make a release a prerelease. The boolean is emitted by tag resolution and passed as a dedicated workflow output to publication. Regression cases cover stable `v1.2.3+build-7`, prerelease `v1.2.3-rc.1+build-7` and major-zero `v0.1.0`. Extracting the former whole-tag policy reproduced one failing case out of 47; the corrected full helper suite passed 47/47 and actionlint passed. Evidence: `artifacts/h04/prerelease-label-{red,green}.log`, `prerelease-actionlint.log` and read-only existing-tag resolution `prerelease-resolve{.json,-output.txt}`. No App, full-suite or package rerun is claimed for this helper/workflow-only correction; native evidence remains attributed to `58b3d11764f35048f1028e8e5bfcccf235669f6b`.

## Review corrections: audit coverage and historical-source eligibility

The verification jobs retain the shipping App audit and also audit the complete Core solution on Windows and complete App solution on Linux and Windows. Each uses the same structured fail-closed JSON/exit/diagnostic validation and retains JSON plus stderr in its job artifact. This restores test-only and platform-dependent dependency coverage; a shipping-only graph is not a substitute.

Release eligibility is deliberately limited to tagged sources containing this verification contract. Resolve inspects the immutable source tree before emitting outputs, requiring the CI workflow, the verification helpers and the App startup smoke files. Missing files produce an actionable failure directing the caller to an eligible tag. This is an early eligibility check; subsequent builds/tests/native report validation remain mandatory. It does not claim that file presence alone proves the smoke works. Historical tags and published releases remain untouched. Running today's helpers against old source cannot create the old executable's missing startup/report behavior; injecting current application code or skipping smoke would violate the immutable-source guarantee, so retroactive release validation is unsupported.

Eligibility RED reproduced 12 failures under the former accept-any-resolvable-source behavior; the full corrected helper suite passed 60/60, including the prerelease-label regressions. Read-only resolution rejected existing `v1.2.4` for missing helpers/StartupSmoke and accepted the current committed source file list. Actual local Windows Core solution, App solution and shipping App audits all passed with empty stderr; the solution reports explicitly contain their test projects. Linux execution is left to hosted CI, not claimed from Windows. Actionlint accepted both workflows. Evidence is `artifacts/h04/source-eligibility-{red,real}.log`, `historical-tag-rejection.log`, `review-fixes-helper-green.log`, `review-fixes-actionlint.log` and `review-{core-solution,app-solution,shipping}-audit.{json,log}`. No App/runtime code changed and no full/native rerun is claimed; native evidence remains at `58b3d11764f35048f1028e8e5bfcccf235669f6b`.
