...
The original implementation produced a completed behavioral RED with 19/19 failures: every malicious endpoint and both unbound bearer paths reached the old permissive behavior. After the phase-one change, the endpoint/capture/refresh-handler set passed 59/59 and the broader Core `Category=DualWrite` set passed 135/135 under `CI=true` Release. Evidence is retained under `artifacts/h05/`. All transports were fake; no live sign-in or endpoint was contacted.
