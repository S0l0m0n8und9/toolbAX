...
- Auth: added optional manual bearer-token mode (user-supplied access token) stored via DPAPI in `profile.db` (fallback env var `FOTB_BEARER_TOKEN`).
...
- Query Builder: fixed crash when loading/saving queries by adding polymorphic JSON support for filter trees (`FilterDto`) and making saved-query operations async-safe; added regression tests for saved query round-tripping.
