...
- WP6 packaging: WiX fragments cover host/plugins/data and Burn bundle is wired for .NET Desktop Runtime 8.0; still need to lock GUIDs/branding and sign outputs.
...
- WP6 packaging refresh: MSI now defaults to per-user under `%LOCALAPPDATA%\FoToolbox`, installs host/plugins/profile.db, and creates a Start Menu shortcut. Burn bundle chains .NET Desktop Runtime 8.0 via registry detection and variables `NetDesktopRuntimeVersion/Exe/Url` plus `FoToolboxMsiPath`. Remaining human actions: supply runtime installer path for builds, lock Product/Upgrade/Bundle codes and branding, and sign MSI/CAB/bundle.
